...
class LIDAR():
    ...
    def filter_low_quality_pts(self):
        ...
        """        
        self.current_scan = [e for e in self.current_scan if e[0]>=self.quality_threshold]
    ...
    def filter_angle(self):
        ...
        if self.min_angle < self.max_angle:
            self.current_scan = [ x for x in self.current_scan if self.min_angle <= x[1] <= self.max_angle ]
        else:
            self.current_scan = [ x for x in self.current_scan if not self.max_angle <= x[1] <= self.min_angle ]
    ...
    def polar_to_cartesian(self):
        ...
        for _,deg,r in self.current_scan:
            
            self.x_points.append(r*math.sin(math.radians(-1*deg)))
            self.y_points.append(-r*math.cos(math.radians(deg)))
    ...
    def stream_distance(self,save_path=None):
        ...
        for scan in self.lidar.iter_scans():
            ...
            distances = [d[2] for d in self.current_scan]
            
            # A MODIFIER
            # Calcul de la moyenne et de l'écart-type des distances extraites
            if len(distances) < 2:
                average_distance = 404
                std_distance = 404
            else : 
                average_distance = stats.mean(distances)
                std_distance = stats.stdev(distances)
    ...
    def proximity_sensor(self,distance_threshold=MAX_RANGE):
        ...
        for scan in self.lidar.iter_scans():
            ...
            if any([e[2] <distance_threshold for e in self.current_scan]):
                buzzer.play(220)
            else:
                buzzer.stop()
