...
class struct_GNSS_coordinate:
  ...
  def __init__(self):
    ...
    self.coords_DD = 0.00 # Coordonnées au format degré décimaux
    self.coords_DMM = "0.00" # Coordonnées au format degré / minute décimales
    self.coords_DMS = "0.00" # Coordonnées au format degré décimaux / minutes / secondes
  ...
  def update_coordinate(self,rslt):
    ...
    if rslt != -1:
      ...
      secondes = self.fractions_minutes / 10 ** 5 * 60
      self.coords_DD = self.degres + self.minutes / 60 + secondes / 3600
      self.coords_DMM = f"{self.degres}{str(self.direction)}{self.minutes + secondes / 60}'"
      self.coords_DMS = f"{self.degres}{str(self.direction)}{self.minutes}'{secondes}\""
