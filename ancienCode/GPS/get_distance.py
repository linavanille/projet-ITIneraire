...
def compute_distance(pt1,pt2):
    ...
    """
    phi_1,lamba_1 = map(m.radians, pt1)
    phi_2,lamba_2 = map(m.radians, pt2)
    D = m.acos( m.sin(phi_1)*m.sin(phi_2) + m.cos(phi_1)*m.cos(phi_2)*m.cos(lamba_1 - lamba_2) ) 
    dist = m.degrees(D) * 111.5 *10**3
    # A COMPLETER
...
def get_distance(position_target):
  """
